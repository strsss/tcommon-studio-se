...
/**
 * We need to override some parts of {@link com.fasterxml.jackson.databind.SerializerProvider} implementation to handle
 * oddities of XML output, like "extra" root element.
 */
public class XmlSerializerProvider extends DefaultSerializerProvider {
    ...
    /**
     * If all we get to serialize is a null, there's no way to figure out expected root name; so let's just default to
     * something like "&lt;null>"...
     */
    ...
    public void serializeValue(JsonGenerator jgen, Object value) throws IOException, JsonProcessingException {
        ...
        if (!_config.isEnabled(SerializationFeature.WRAP_ROOT_VALUE)) {
            ...
            Set<Entry<String, Object>> entrySet = mapValue.entrySet();
            if (entrySet.size() == 1) {
                Entry<String, Object> entry = entrySet.iterator().next();
                rootName = new QName("", "" + entry.getKey()); //$NON-NLS-1$//$NON-NLS-2$
                serializeValue = entry.getValue();
            }
        }
        _initWithRootName(jgen, rootName);
        super.serializeValue(jgen, serializeValue);
    }
    ...
    protected void _initWithRootName(JsonGenerator jgen, QName rootName) throws IOException, JsonProcessingException {
        ...
        /*
         * 28-Nov-2012, tatu: We should only initialize the root name if no name has been set, as per [Issue#42], to
         * allow for custom serializers to work.
         */
        ...
        /*
         * [Issue-26] If we just try writing root element with namespace, we will get an explicit prefix. But we'd
         * rather use the default namespace, so let's try to force that.
         */
